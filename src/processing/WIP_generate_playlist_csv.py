...
if not spotify_available:
    from src.processing.csv_playlist_generation import generate_backup_playlist
...
def fetch_mood_tracks(mood_keywords_string: str, num_tracks: int):
    ...
    attempts = [
        (mood_keywords_string, "full", "Most Specific (Fixed Base + Moods)"), 
        (mood_keywords_string, "mood_only", "Intermediate (Mood Only)"),
        (FALLBACK_GENRE, "mood_fallback", "Safe Fallback (Ambient Only)")
    ]
    
    for i, (current_moods, query_type, attempt_name) in enumerate(attempts):
...
# --- NEW FUNCTION: Adjusts playlist duration against estimated read time ---
def adjust_playlist_duration(tracks: list, target_time_min: float, mood_keywords_string: str) -> list:
    ...
    if diff_min > 5:
        ...
        # Sort by duration descending to remove the longest tracks first
    ...
    # --- Case B: Playlist is Too Short (Add Songs) ---
        ...
        # Estimate how many tracks to add (based on the average song length)
        ...
        # Use the safest search method (Fallback) to get more tracks
        # Note: We fetch twice the amount needed in case of duplicates
        raw_new_tracks, query = _run_batched_search(_build_query(FALLBACK_GENRE, "fallback"), num_to_add * 2) 

        # Filter new tracks for uniqueness and add them
...
def generate_playlist():
    ...
    # Step 2: Compute total track target and estimated read time
    ...
    for attempt in range(1, 3):
        ...
        # 3a. Re-fetch the Gemini output only if this is the retry attempt (not the initial one)
        if attempt == 2:
            print("Retrying Gemini call to fetch fresh response...")
            try:
                data = generate_playlist_input(suppress_input=True) # Assuming input function can suppress prompts
                gemini_response_str = data.get("response", "{}")
            except Exception as e:

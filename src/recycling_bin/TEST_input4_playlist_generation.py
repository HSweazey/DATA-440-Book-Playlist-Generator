...
import ast
import time
from generate_playlist_input4_dummy import generate_playlist_input
import spotipy
...
# NOTE: INSTRUMENTAL_THRESHOLD and the filter_tracks_by_instrumentalness function are removed.
...
def _build_query(genre: str, mood_keywords_string: str, query_type: str) -> str:
    ...
    ANTI_VOCAL_KEYWORDS = "instrumental no vocals" 
...
def fetch_search_tracks(genre: str, mood_keywords_string: str, num_tracks: int):
    ...
    This function now returns the raw tracks found, relying on aggressive keywords for filtering.
    """
    
    # Define the three cascading attempts
    attempts = [
        # Attempt 1: Most Specific (All Keywords)
        (genre, mood_keywords_string, "full", "Most Specific (Full Query)"), 
    ...
    for i, (current_genre, current_moods, query_type, attempt_name) in enumerate(attempts):
        ...
        # We fetch the target number of tracks, no extra needed since filtering is internal to the search
...
def generate_playlist():
    ...
    tracks, final_query = fetch_search_tracks(primary_genre, mood_keywords_string, num_tracks_target)
    ...
    print(f"Tracks Found: {len(tracks)} (Targeted: {num_tracks_target})")
